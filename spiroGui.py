...
class SpiroGui():
    ...
    def __init__(self):
        ...
        self.addButton = tk.Button(
            self.optionFrame, text="Add new...", command=self.addSpiroState)
        ...
        self.deleteButton = tk.Button(
            self.optionFrame, text="Delete", command=self.deleteSpiroState)
        ...
        self.dialogLabel.configure(
            text="No spirolaterals saved. Please create a new one.")
        ...
        self.prevCancelButton = tk.Button(
            self.controlFrame, text="<- Prev", command=self.previousSpiro)
        ...
        self.currentSpiroMultipleLabel = tk.Label(
            self.controlFrame, text="Multiple: ")
        # Put this between the prevCancel button and the addConfirm button
        ...
        self.nextConfirmButton = tk.Button(
            self.controlFrame, text="Next ->", command=self.nextSpiro)
        ...
        self.spiroDrawer = spiroModule.SpirolateralDrawer(
            self.turtleScreen, 10)
    ...
    def normalState(self):
        ...
        if len(self.spiroList) == 0:
            ...
            self.dialogLabel.configure(text="Displaying spirolateral {0} of {1}".format(
                self.currentSpiroIndex + 1, len(self.spiroList)))
        ...
        self.currentSpiroNameEntry.configure(
            state="readonly", textvariable=self.currentSpiroNameText, fg="black")
        self.currentSpiroMultipleEntry.configure(
            state="readonly", textvariable=self.currentSpiroMultipleText, fg="black")
        ...
        self.prevCancelButton.configure(
            text="<- Prev", command=self.previousSpiro)
        self.nextConfirmButton.configure(
            text="Next ->", command=self.nextSpiro)
    ...
    def addSpiroState(self):
        ...
        self.prevCancelButton.configure(
            text=" Cancel", command=self.normalState, state="normal")
        self.nextConfirmButton.configure(
            text="Confirm", command=self.addNewSpirolateral, state="normal")
        ...
        self.dialogLabel.configure(
            text="Please enter the new spirolateral's name and multiple below")
        ...
        self.currentSpiroNameEntry.configure(
            state="normal", fg="black", textvariable=self.currentSpiroNameText)
        self.currentSpiroMultipleEntry.configure(
            state="normal", fg="black", textvariable=self.currentSpiroMultipleText)
        self.currentSpiroMultipleText.set("")
        self.currentSpiroNameText.set("")
    ...
    def deleteSpiroState(self):
        ...
        self.nextConfirmButton.configure(
            text="  Yes  ", command=self.deleteSpiro)
        self.prevCancelButton.configure(
            text="  No  ", command=self.normalState)
    ...
    def addNewSpirolateral(self):
        ...
        if not invalidEntries:
            ...
            self.spiroList.append(spiroModule.Spirolateral(
                newSpiroName, newSpiroMultiple, 90))
